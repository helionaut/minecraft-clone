...
export function buildWindowsRuntimeReadme(plan) {
  return [
    ...
    '  startup-profile-upload-bundle.zip (best effort when PowerShell is available)',
    '',
...
export function buildWindowsRuntimeCommand(plan) {
  return [
    ...
    `  if exist "${plan.windowsRuntimeDir}\\artifacts\\startup-profile-upload-bundle.zip" del /f /q "${plan.windowsRuntimeDir}\\artifacts\\startup-profile-upload-bundle.zip"`,
    `  "%SystemRoot%\\System32\\WindowsPowerShell\\v1.0\\powershell.exe" -NoProfile -ExecutionPolicy Bypass -Command "Compress-Archive -Path '${plan.windowsRuntimeDir}\\artifacts\\*' -DestinationPath '${plan.windowsRuntimeDir}\\artifacts\\startup-profile-upload-bundle.zip' -Force"`,
    '  if errorlevel 1 echo [webgpu-startup-profile] warning: failed to create startup-profile-upload-bundle.zip',
    ') else (',
    '  echo [webgpu-startup-profile] warning: PowerShell is unavailable; skipping startup-profile-upload-bundle.zip creation',
    ')',
    'endlocal',
    '',
  ].join('\r\n');
