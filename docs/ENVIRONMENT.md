...
- Remote workflow readiness update: the repo now includes `.github/workflows/profile-webgpu-startup.yml`, a `workflow_dispatch` lane that targets a future self-hosted Windows/GPU runner with labels `self-hosted`, `windows`, `x64`, and `gpu`. It runs `npm run profile:webgpu-startup:local-preview` and uploads `reports/startup-profiling` as a GitHub Actions artifact.
- Dispatchability constraint: GitHub only exposes `workflow_dispatch` workflows from the default-branch workflow registry. A direct `gh api .../actions/workflows/profile-webgpu-startup.yml/dispatches` call against this repo currently returns `404` because the workflow file exists only on PR branch `eugeniy/hel-142-profile-desktop-frame-spikes-on-rtx-chrome-for-webgpu-scene`, not on `main` yet.
- CLI confirmation: `gh workflow run profile-webgpu-startup.yml --ref eugeniy/hel-142-profile-desktop-frame-spikes-on-rtx-chrome-for-webgpu-scene` also fails with `HTTP 404: workflow profile-webgpu-startup.yml not found on the default branch`, which confirms there is no hidden branch-local dispatch path through the GitHub CLI while the workflow remains absent from `main`.
- Current implication: even if a suitable self-hosted Windows/GPU runner appeared immediately, this repo still could not launch the profiling workflow through GitHub Actions until the workflow file is present on `main` or equivalent default-branch state.
