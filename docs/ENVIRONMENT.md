...
- Current implication: the remaining blocker is the missing eligible GPU execution surface, not more repo wiring. The truthful execution paths are either a self-hosted Windows/x64/GPU runner for this repo plus manual dispatch after merge/default-branch registration, or an uploaded RTX capture artifact bundle from outside GitHub Actions. GitHub-hosted larger runners do not currently rescue this repo because the owner is a user account rather than an eligible organization.
