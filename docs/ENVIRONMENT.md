...
- Windows host probe update from WSL:
  ...
  - this Linux guest also has no native `google-chrome`, `chromium`, or `chromium-browser` in PATH, and a direct Node probe reports `navigator.gpu === false`, so there is no truthful Linux-side WebGPU fallback surface either
  ...
  - invoking Windows binaries through `/init` does not help either; `/init /mnt/c/Windows/System32/cmd.exe /c ver` and `/init /mnt/c/Program Files/Google/Chrome/Application/chrome.exe --version` fail with the same `Invalid argument`, so the host-side Windows launch lane is exhausted from this session rather than merely misconfigured at the shell level
  ...
  - the repo now includes `npm run profile:webgpu-startup:stage-windows-runtime` to regenerate that Windows-local runtime bundle, including a `run-startup-profile.cmd` launcher, `README.txt`, the helper/report/compare scripts, and the committed Intel control baseline, from WSL before handing off to Windows-side execution
  ...
  - the staged launcher now also generates `startup-profile-report.*`, `startup-profile-comparison.*`, and `startup-profile-upload-manifest.*` inside its Windows-side artifact directory, and it attempts to zip the whole artifact set as `startup-profile-upload-bundle.zip` when PowerShell is available, so the next same-OS run does not depend on a second manual post-processing step after capture
...
- GitHub runner limitation: the repository currently has zero registered self-hosted Actions runners, and the only CI workflow for PR branches runs on `ubuntu-latest`, so there is no currently usable repo-managed remote GPU execution path from GitHub.
- GitHub-hosted GPU limitation update: GitHub now documents GPU-powered larger runners, but those are limited to organizations and enterprises on Team or Enterprise Cloud plans. This repository is owned by the user account `helionaut`, not an organization, so that hosted GPU option is not available to this repo in its current ownership model.
- Current queued-run evidence: PR #52 now points at `8e34ed9eb5b51fb8b29156528e3697c7d195bc6c`, `CI / check` is green, and the previously published `Profile WebGPU Startup` run `23788134222` is still queued because `gh api repos/helionaut/minecraft-clone/actions/runners` returns `total_count=0`.
- Remote workflow readiness update: the repo includes `.github/workflows/profile-webgpu-startup.yml`, a profiling lane that targets a future self-hosted Windows/GPU runner with labels `self-hosted`, `windows`, `x64`, and `gpu`. A 2026-04-01 cleanup removed its issue-branch `push` and `pull_request` triggers again because, with zero eligible runners, those auto-triggers only left a permanently queued PR check; the truthful remote path is manual `workflow_dispatch` after the workflow lands on `main` and a runner exists.
- Dispatchability update: GitHub now registers `Profile WebGPU Startup` in the workflow registry, and `gh workflow run "Profile WebGPU Startup" --ref eugeniy/hel-142-profile-desktop-frame-spikes-on-rtx-chrome-for-webgpu-scene -f ref=eugeniy/hel-142-profile-desktop-frame-spikes-on-rtx-chrome-for-webgpu-scene -f require_rtx=true` succeeded on 2026-04-01, creating run `23825143229`.
- Execution-capacity confirmation: that manually dispatched run created job `profile` / `69446289159` but stayed `queued` until cancellation because `gh api repos/helionaut/minecraft-clone/actions/runners` still returns `total_count=0`.
- Current implication: the remaining blocker is now strictly missing eligible GPU execution capacity, not missing workflow dispatchability. The truthful execution paths are either a self-hosted Windows/x64/GPU runner for this repo plus manual dispatch of `Profile WebGPU Startup`, or an uploaded RTX capture artifact bundle from outside GitHub Actions. GitHub-hosted larger runners do not currently rescue this repo because the owner is a user account rather than an eligible organization.
...
- Artifact-input limitation: there is still no uploaded RTX startup capture bundle in this workspace or PR context; `reports/startup-profiling/` is empty and only the committed Intel control baseline exists under `artifacts/startup-profiling-baselines/`.
