...
- Windows host probe update from WSL:
  ...
  - the staged launcher now also generates `startup-profile-report.*`, `startup-profile-comparison.*`, and `startup-profile-upload-manifest.*` inside its Windows-side artifact directory, so the next same-OS run does not depend on a second manual post-processing step after capture and already includes the upload checklist
