...
- Current queued-run evidence: PR #52 now points at `a3504e9d5bd8327a986db9e1438580e7c85e7562`, `CI / check` is green, and `Profile WebGPU Startup` run `23787701789` remains queued because `gh api repos/helionaut/minecraft-clone/actions/runners` still returns `total_count=0`.
...
- Current implication: the default-branch registration gate is no longer the only repo-side path because the issue branch can now auto-run the profiling job on a future eligible `push` or profiling-related PR update. The remaining blocker is the missing eligible GPU execution surface: either a self-hosted Windows/x64/GPU runner for this repo, or an uploaded RTX capture artifact bundle from outside GitHub Actions. GitHub-hosted larger runners do not currently rescue this repo because the owner is a user account rather than an eligible organization.
