...
- Windows host probe update from WSL:
  ...
  - the staged launcher now also generates `startup-profile-report.*` and `startup-profile-comparison.*` inside its Windows-side artifact directory, so the next same-OS run does not depend on a second manual post-processing step after capture
