...
- Current queued-run evidence: PR #52 now points at `5609adff6bbdc09d5d5bb1c0b9636592342ba5ad`, `CI / check` is green, and `Profile WebGPU Startup` run `23787292496` remains queued because `gh api repos/helionaut/minecraft-clone/actions/runners` still returns `total_count=0`.
...
- Current implication: the default-branch registration gate is no longer the only repo-side path because the issue branch can now auto-run the profiling job on a future eligible `push` or profiling-related PR update. The remaining blocker is the missing self-hosted Windows/x64/GPU runner or uploaded RTX capture artifact, not a missing branch-local event trigger.
